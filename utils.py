...
class Evaluate():
    ...
    @staticmethod
    def _GetMajorityVote(list_of_labels: List[List[int]]) -> List[int]:
        """
    ...
    def PairwiseComparison(self, label_arr: List[List[Any]], include_majority=True, visualize=True, y_labels=None) -> List[List[bool]]:
        """
        ...
        Args:
            label_arr: A list containing the lists of labels generated by models to compare.
        ...
        if include_majority:
            label_arr.append(self._GetMajorityVote(label_arr))

        for list1, list2 in combinations(label_arr, 2):
            comparison = [x == y for x, y in zip(list1, list2)]
            result.append(comparison)
        if visualize and y_labels is not None:
            if include_majority:
                y_labels.append("majority vote")
            self.LabelAgreementGraph(result, y_labels)
            return result
        elif visualize and y_labels is None:
            raise ValueError(f"When visualize==True you need to pass label names for y axis.")
        else:
            return result
    

    @staticmethod
    def LabelAgreementGraph(arr, y_labels):
